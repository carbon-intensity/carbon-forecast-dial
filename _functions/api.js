...
const getTimeBounds = (time, duration) => {
    let rtrn = {
        begin: moment(time).utc(),
        finish: moment(time).utc().add(duration, 'hours'),
...
const getCarbonForecast = (start, end) => {
    return new Promise( (resolve, reject) => {
        ...
        const endpoint = `https://api.carbonintensity.org.uk/intensity/stats/${startISO}/${endISO}/1`;
...
exports.handler = (event, context, callback) => {
    ...
    let timeBounds = getTimeBounds(timeInUK, duration);

    // Request the data from the API
    getCarbonForecast(timeBounds.begin, timeBounds.finish)
        .then( (response) => {
